  ...
  <div id="hy-swiper">
    ...
    <div class="indicator">
      <slot name="indicator" v-if="showIndicator && slideCount>1">
        <div v-for="(item, index) in slideCount" class="indi-item" @click="clickIndicator(index)"
...
export default {
  ...
  methods: {
    ...
    handleDom() {
      ...
      // 让swiper元素，显示第一个（木齐去哪是显示前面添加的最后一个元素)
    ...
    touchEnd(e) {
      ...
      else if(this.distance<0 && currentMove>this.totalWidth*this.moveRatio) { //享有移动超过0.5
        this.currentIndex++
      }
      // 移动到正确的位置
      this.scrollContent(-this.currentIndex * this.totalWidth);
